...
export function ProfitMetrics({ locale }: { locale: Locale }) {
  return (
    <section
      id="economics"
    ...
    >
      <div className="text-center mb-16">
        <h2 data-anim="title" className="text-3xl sm:text-4xl font-black text-primary">
          {t(locale, landingCopy.economics.title)}
        </h2>
        <p data-anim="subtitle" className="text-on-surface-variant">
          {t(locale, landingCopy.economics.subtitle)}
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {landingCopy.economics.metrics.map((m) => (
          <Metric
            key={m.value}
            value={m.value}
